    ...
    )
{
    ...
    const float4 input_color = read_imagef(input, tex_sampler, gid);

    if (input_color.w < THRESHOLD) {
        ...
        for (int32_t x = -width; x < width + 1; ++x) {
            for (int32_t y = -width; y < width + 1; ++y) {
                ...
                if (1 || (point_position.x == current_position.x || point_position.y == current_position.y || point_position.z == current_position.z)) {
        ...
        write_imagef(output, gid, input_color);
    }
}
