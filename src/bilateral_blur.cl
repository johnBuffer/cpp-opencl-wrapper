    ...
    )
{
    ...
    const float4 temporal_data = read_imagef(input, tex_sampler, gid);
    if (temporal_data.w) {
        const float3 current_color    = temporal_data.xyz;
        const float4 current_data     = read_imagef(ss_positions, tex_position_sampler, gid);
        ...
        for (int32_t x = -width; x < width + 1; ++x) {
            for (int32_t y = -width; y < width + 1; ++y) {
                ...
                const float3 other_color = read_imagef(input, tex_sampler, gid + coord_off).xyz;
                const float4 other_data = read_imagef(ss_positions, tex_position_sampler, gid + coord_off);
    ...
    )
{
    ...
    write_imagef(output, gid, (float4)(sum / 9.0f));
}
