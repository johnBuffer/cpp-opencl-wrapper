    ...
    )
{
    ...
    for (int i=-5; i<6; ++i) {
        ...
        sum += kernel_value;
        color += read_imagef(input, tex_sampler, gid + (int2)(0, i)).xyz * kernel_value;
    }
    color /= sum;

    write_imagef(output, gid, (float4)(color, last_coord));
}
    ...
    )
{
    ...
    for (int i=-5; i<6; ++i) {
        const float kernel_value = KERNEL[abs(i)];
        sum += kernel_value;
        color += read_imagef(input, tex_sampler, gid + (int2)(i, 0)).xyz * kernel_value;
    }
    color /= sum;
